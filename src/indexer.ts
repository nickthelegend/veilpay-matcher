...
export class IndexerService {
  ...
  constructor(config: MatcherConfig, matcher: any) {
    this.config = config;
  ...
  private async handleClientMessage(
    ...
    switch (msg.type) {
      ...
      case "submitOrder": {
        if (this.matcher && msg.data) {
          const result = await this.matcher.handleEncryptedOrder(msg.data);
  ...
  private getStateSnapshot(): object {
    ...
    return {
      ...
      stats: this.getStats(),
      ecdhPublicKey: this.matcher?.encryption?.getPublicKeyHex(),
    };
  }
