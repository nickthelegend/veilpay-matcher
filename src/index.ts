...
class DarkBookMatcher {
  ...
  constructor() {
    ...
    this.config = {
      rpcUrl: process.env.CONFLUX_RPC_URL || "https://testnet.conflux.validationcloud.io/v1/bC52X43z8nneoh11p4JiDs7eQLLw1rqv4URTn-AOpfg",
    ...
    this.indexer = new IndexerService(this.config, this);
  }
   ...
   */
  async handleEncryptedOrder(data: any): Promise<{
    success: boolean;
    matches: Match[];
    error?: string;
  }> {
    try {
      // Convert arrays back to Uint8Array if they came via JSON
      const encrypted: EncryptedOrder = {
        ciphertext: data.ciphertext instanceof Uint8Array ? data.ciphertext : new Uint8Array(data.ciphertext),
        nonce: data.nonce instanceof Uint8Array ? data.nonce : new Uint8Array(data.nonce),
        ephemeralPubKey: data.ephemeralPubKey instanceof Uint8Array ? data.ephemeralPubKey : new Uint8Array(data.ephemeralPubKey),
